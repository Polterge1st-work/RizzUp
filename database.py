...
async def get_user_stats(user_id: int) -> dict:
    ...
    async with pool.acquire() as conn:
        total = await conn.fetchval(
            "SELECT COUNT(*) FROM requests WHERE user_id = $1", user_id
        )
        by_feature = await conn.fetch(
            "SELECT feature, COUNT(*) as cnt FROM requests WHERE user_id = $1 GROUP BY feature",
            user_id
        )
    return {
        "total": total,
...
async def cleanup_old_requests(days: int = 90):
    """Удаляет записи запросов старше N дней."""
    pool = await get_pool()
    async with pool.acquire() as conn:
